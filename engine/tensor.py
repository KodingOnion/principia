...
class Tensor:
    """Numpy-backed tensor with minimal reverse-mode autodiff support.

    This class mimics a tiny autograd Tensor: it stores ``data`` and ``grad``
    arrays, builds a computation graph via child references, and provides
    elementary tensor operations with corresponding local backward rules.
    """
    ...
    def __add__(self, other):
        ...
        result = Tensor(data=(self.data + other.data), children=[self, other], op="+")
    ...
    def __pow__(self, exponent):
        if isinstance(exponent, (int, float)):
            result = Tensor(data=(self.data ** exponent), children=[self], op='**')
    ...
    def exp(self):
        result = Tensor(data=np.exp(self.data), children=[self], op='exp')
    ...
    def sum(self, axis=None, keepdims=False):
        result = Tensor(data=np.sum(self.data, axis=axis, keepdims=keepdims), children=[self], op="sum")
    ...
    def reshape(self, new_shape):
        result = Tensor(data=self.data.reshape(new_shape), children=[self], op="reshape")
